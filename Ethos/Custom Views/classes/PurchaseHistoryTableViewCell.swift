...
class PurchaseHistoryTableViewCell: UITableViewCell {
    ...
    var purchaseHistoryData : GetPurchaseHistoryData? {
        didSet {
            if (self.purchaseHistoryData?.invoiceAttachmentPath ?? "").isValidURL{
                ...
                self.downloadInvoiceImg.image = UIImage(named: "download_btn")
            }else{
                self.downloadInvoiceImg.isHidden = true
                self.downloadInvoiceImg.image = UIImage(named: "")
            }
    ...
    override func awakeFromNib() {
        ...
        mainBackView.layer.borderColor = UIColor(red:0.9176470588, green:0.9176470588, blue:0.9176470588, alpha: 1).cgColor
