//
//  AdCollectionReusableView.swift
//  Ethos
//
...
class AdCollectionReusableView: UICollectionReusableView {
...
extension AdCollectionReusableView {
...
extension AdCollectionReusableView : GetVideoModelDelegate {
    func didGetUrl(url: String, image: String, index : IndexPath) {
        ...
        DispatchQueue.main.async {
            if let url = URL(string: url) {
                ...
                if (self.superCollectionView?.visibleSupplementaryViews(ofKind: UICollectionView.elementKindSectionFooter).contains(self) ?? false) && UIApplication.topViewController() == self.superViewController , let index = self.index,
                   let attributes = self.superCollectionView?.layoutAttributesForSupplementaryElement(ofKind: UICollectionView.elementKindSectionFooter, at: index),
                   let rectOfCellInSuperview = self.superCollectionView?.convert(attributes.frame, to: self.superCollectionView?.superview), rectOfCellInSuperview.origin.y < self.frame.height - 100, rectOfCellInSuperview.origin.y > 50 {
